...
IMPORTANT: Return ONLY the JSON object, no markdown code blocks, no explanatory text.`;
      ...
      try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a helpful assistant that responds only with valid JSON objects. Never include markdown formatting or explanatory text.',
            },
            {
              role: 'user',
              content: prompt,
        ...
        if (
          formatError.message?.includes('response_format') ||
          formatError.code === 'invalid_request_error'
        ) {
          console.log(
            `Model ${selectedModel} doesn't support response_format, retrying without it...`
          );
          completion = await openai.chat.completions.create({
            model: selectedModel,
            messages: [
      ...
      if (error.message?.includes('JSON')) {
        throw new Error(
          `AI model returned invalid JSON format. Please try again or use a different model.`
        );
      }
  ...
  private parseAIResponse(
    ...
    try {
      ...
      if (!jsonMatch) {
        console.error(
          `No JSON found in AI response from model ${modelId}:`,
      ...
      if (missingFields.length > 0) {
        console.error(
          `Missing fields in AI response from model ${modelId}:`,
          missingFields
        );
      ...
      if (
        !Array.isArray(parsed.thumbnailIdeas) ||
        parsed.thumbnailIdeas.length === 0
      ) {
        throw new Error('AI response must include an array of thumbnail ideas');
      }
      if (
        !Array.isArray(parsed.scriptOutline) ||
        parsed.scriptOutline.length === 0
      ) {
        throw new Error(
          'AI response must include an array of script outline sections'
        );
      }
      if (
      ...
      console.error(
        `Error parsing AI response from model ${modelId}:`,
        error.message
      );
