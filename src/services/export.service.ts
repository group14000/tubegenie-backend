...
export class ExportService {
  ...
  generatePDF(content: IContent): PDFDoc {
    ...
    content.titles.forEach((title, index) => {
      doc.text(`${index + 1}. ${title}`, { indent: 20 }).moveDown(0.3);
    });
    doc.moveDown(0.7);
    ...
    content.thumbnailIdeas.forEach((idea, index) => {
      doc.text(`${index + 1}. ${idea}`, { indent: 20 }).moveDown(0.3);
    });
    doc.moveDown(0.7);
    ...
    content.scriptOutline.forEach((section, index) => {
      doc.text(`${index + 1}. ${section}`, { indent: 20 }).moveDown(0.3);
    });
  ...
  generateMarkdown(content: IContent): string {
    ...
    lines.push('---');
    lines.push(
      `*Generated by ${content.aiModel} on ${new Date(content.createdAt).toLocaleString()}*`
    );
